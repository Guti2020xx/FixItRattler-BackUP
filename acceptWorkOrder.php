<?php
    $ID = $_POST["id"];
    $Date = $DateSub = date("Y/m/d H:i:s", time()); 
    ...
    {   // asddasdasdasd
        $sql = "UPDATE WorkOrderInfo 
                SET WorkOrderStatus = 'Accepted', DateAccepted = ? 
                WHERE ID = ?";
        ...
        if($results)
        {
            ...
            $body = "Work Order #$ID has been accepted by facilities. Track your progess!";
            ...
            echo "<script>
            alert(' Work Order with ID=".$ID." was accepted ');
